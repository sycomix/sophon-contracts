...
import './interfaces/ISophonQuickConverter.sol';
import './interfaces/IEtherToken.sol';
import './interfaces/ISophonToken.sol';
import './interfaces/ITokenConverter.sol';

/*
    The SophonQuickConverter contract provides allows converting between any token in the 
    sophon network in a single transaction.

    A note on conversion paths -
    Conversion path is a data structure that's used when converting a token to another token in the sophon network
    ...
    in which each 'hop' is represented by a 2-tuple - sophon token & to token.
    In addition, the first element is always the source token.
    The sophon token is only used as a pointer to a converter (since converter addresses are more likely to change).

    Format:
    [source token, sophon token, to token, sophon token, to token...]
*/
contract SophonQuickConverter is ISophonQuickConverter, TokenHolder {
    mapping (address => bool) public etherTokens;   // list of all supported ether tokens

    /**
        @dev constructor
    */
    function SophonQuickConverter() {
    }
    ...
    */
    ...
    /**
        @dev converts the token to any other token in the sophon network by following
    ...
    */
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 1; i < pathLength; i += 2) {
            sophonToken = ISophonToken(_path[i]);
            toToken = _path[i + 1];
            converter = ITokenConverter(sophonToken.owner());

            // if the sophon token isn't the source (from token), the converter doesn't have control over it and thus we need to approve the request
            if (sophonToken != fromToken)
    ...
    /**
        @dev claims the caller's tokens, converts them to any other token in the sophon network
