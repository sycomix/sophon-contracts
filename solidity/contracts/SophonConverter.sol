...
import './SophonTokenController.sol';
import './Managed.sol';
import './Utils.sol';
...
/*
    Sophon Converter v0.6

    The Sophon version of the token converter, allows conversion between a sophon token and other ERC20 tokens and between different ERC20 tokens and themselves.
    ...
    The converter is upgradable (just like any SophonTokenController).

    WARNING: It is NOT RECOMMENDED to use the converter with sophon tokens that have less than 8 decimal digits
...
*/
contract SophonConverter is ITokenConverter, SophonTokenController, Managed {
    ...
    struct Connector {
        ...
        bool isPurchaseEnabled;         // is purchase of the sophon token enabled with the connector, can be set by the owner
        bool isSet;                     // used to tell if the mapping element is defined
    }

    string public version = '0.6';
    string public converterType = 'sophon';

    ISophonConverterExtensions public extensions;       // sophon converter extensions contract
    ...
    /**
        ...
        @param  _token              sophon token governed by the converter
        @param  _extensions         address of a sophon converter extensions contract
        @param  _maxConversionFee   maximum conversion fee, represented in ppm
        @param  _connectorToken     optional, initial connector, allows defining the first connector at deployment time
        @param  _connectorWeight    optional, weight for the initial connector
    */
    function SophonConverter(ISophonToken _token, ISophonConverterExtensions _extensions, uint32 _maxConversionFee, IERC20Token _connectorToken, uint32 _connectorWeight)
    ...
    */
    ...
    /**
        ...
        note that the number of convertible tokens is the number of connector token, plus 1 (that represents the sophon token)
    ...
    */
    ...
    /*
        ...
        @param _extensions    address of a sophon converter extensions contract
    */
    function setExtensions(ISophonConverterExtensions _extensions)
        public
        ownerOnly
        validAddress(_extensions)
        notThis(_extensions)
    ...
    /*
        ...
        @param _path    new quick buy path, see conversion path format in the SophonQuickConverter contract
    */
    ...
    */
    ...
    /**
        ...
        @param _connectorToken  connector token contract address
        @param _sellAmount      amount to sell (in the sophon token)
    ...
    */
        ...
        returns (uint256)
    {
        ...
        // issue new funds to the caller in the sophon token
    ...
    */
        ...
        returns (uint256)
    {
        ...
        // destroy _sellAmount from the caller's balance in the sophon token
    ...
    /**
        @dev converts the token to any other token in the sophon network by following a predefined conversion path
        note that when converting from an ERC20 token (as opposed to a sophon token), allowance must be set beforehand

        @param _path        conversion path, see conversion path format in the SophonQuickConverter contract
    ...
    */
        ...
        returns (uint256)
    {
        ...
        ISophonQuickConverter quickConverter = extensions.quickConverter();
        ...
        if (msg.value == 0) {
            ...
            // if the token is the sophon token, no allowance is required - destroy the tokens from the caller and issue them to the quick converter
            if (fromToken == token) {
                token.destroy(msg.sender, _amount); // destroy _amount tokens from the caller's balance in the sophon token
    ...
    */
    ...
    /**
        @dev fallback, buys the sophon token with ETH
