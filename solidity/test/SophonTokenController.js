...
contract('SophonTokenController', (accounts) => {
    ...
    it('verifies the token address after construction', async () => {
        let controller = await SophonTokenController.new(tokenAddress);
    ...
    it('should throw when attempting to construct a controller with no token', async () => {
        try {
            await SophonTokenController.new('0x0');
    ...
    it('verifies that the owner can transfer token ownership', async () => {
        ...
        let controller2 = await SophonTokenController.new(token.address);
